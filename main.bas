...
160 print"{home}{10 rght}bombs:"c"{left} ":return
...
300 r=int(t-ti/60):ifr<1then510
310 print"{home}time:"r"{left} "
315 sys(ml+12):sys(ml+0)
320 ifr<9thenpoke36877,253
330 if(peek(37137)and32)=0then760
350 ifqandti>1087thensys(ml+18):q=0:sys(ml+03),2:sys(ml+15)
355 ifst=1then510
357 ifst=2thenf=1:poke144,0:poke36874,250:pokew,32:gosub150:c=c+1:ifc=2thenc=0:goto400
340 iffthenf=f+1:iff=4thenf=0:poke36874,0
370 gosub170:hp=hp+h
380 pokehp,115::ifh<>0thenpokehp-h,32
390 pokew,116:goto300

400 fori=0to500:next
401 poke822,0:poke36874,0:poke36877,0:ifpeek(36875)=0then403
402 ifpeek(36875)<>1then402
403 gosub1240
404 poke646,(rnd(0)*6)+2:sys(ml+21):print"{wht}"
405 sys(ml+03),1:sys(ml+24)
406 ifpeek(36875)<>1then406
408 sys(ml+18):sys(ml+03),2
410 a$="                         "
411 a$=a$+"loading...            "
...
480 gosub970:gosub150
490 sys(ml+06):sys(ml+15)
500 goto290
...
550 sys(ml+03),0
560 gosub970:gosub150
570 poke144,0:q=1:c=0:poke820,0:sys(ml+15):goto290
...
920 forj=0to7:poke36874,j+128:next:poke36874,0:return
...
970 restore:fori=0to7:reada:poke832+i,a:next:return
